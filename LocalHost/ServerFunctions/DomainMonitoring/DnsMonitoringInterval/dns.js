...
async function dnsMonitoring(domain, transporter) {
  ...
  const newObj = {
    ...
    nameServer: whois?.["Name Server"] || whois?.["Name servers"] || "n/a",
  };
  ...
  for (let key in newObj) {
    if (newObj.hasOwnProperty(key) && oldObj[key] !== newObj[key]) {
      changes.push(`${key} from ${oldObj[key]} to ${newObj[key]}`);
    }
  }

  if (changes.length > 0) {
    console.log(`${newObj["domain"]} changed ${changes.join(" and ")}`);
  } else {
    console.log("No changes detected.");
  }
}
module.exports = dnsMonitoring;
